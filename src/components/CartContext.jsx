...
function mergeItems(prev, next) {
  // combine por productId + variantId + size
...
export function CartProvider({ children }) {
  ...
  const addItem = (item) => setItems(prev => mergeItems(prev, [item]));
  const removeItem = (productId, { variantId = undefined, size = undefined } = {}) =>
    setItems(prev =>
      prev.filter(i => !(i.productId === productId && (i.variantId ?? "") === (variantId ?? "") && (i.size ?? "") === (size ?? "")))
    );
  const setQty = (productId, qty, { variantId = undefined, size = undefined } = {}) =>
    setItems(prev =>
      prev.map(i =>
